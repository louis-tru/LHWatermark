...
+ (UIImage*) imageWidthText:(NSString*) text
	...
	lb.text = text;
	lb.font = font;
	lb.bounds = [text boundingRectWithSize:CGSizeMake(MAXFLOAT, MAXFLOAT)
...
+ (UIImage*) restoreImageWith:(unsigned) seed
...
+ (UIImage*) imageFromRGB:(void*) rawData
										width:(NSUInteger) w
									 height:(NSUInteger) h {
...
- (UIImage*) resizeImageWith:(NSInteger) width
											height:(NSInteger) height {
